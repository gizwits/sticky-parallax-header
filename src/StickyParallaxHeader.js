...
class StickyParallaxHeader extends Component {
  ...
  render() {
    ...
    const { currentPage, isFolded, height } = this.state;
    const scrollHeight = Math.max(parallaxHeight, headerHeight * 2);
    const headerStyle = header.props.style;
    const isArray = Array.isArray(headerStyle);
    const arrayHeaderStyle = {};
    ...
    return (
      <View style={styles.container}>
        <ImageBackground source={backgroundImage} style={{position: 'absolute',width:'100%',height:height}}></ImageBackground>
        ...
        <AnimatedScrollView
          ...
          onScroll={event(
            ...
            ],
            {
              useNativeDriver: true,
              listener: (e) => {
                console.log('onScrroll========',300-e.nativeEvent.contentOffset.y)
                this.setState({height: 300-e.nativeEvent.contentOffset.y>100?300-e.nativeEvent.contentOffset.y:100})
